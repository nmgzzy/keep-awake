...
- 单实例运行：重复启动时第二个进程会静默退出，不会出现多个托盘图标
  （Windows 用命名互斥量，mac/Linux 用文件锁）。
- 「开机自启」开关（Windows：写 `HKCU\...\Run`；mac/Linux 暂未实现，菜单项置灰）。
- 进程退出自动释放系统锁，不会残留。

Windows 版静态链接 CRT，**仅依赖系统 DLL**（USER32/SHELL32/KERNEL32/GDI32/ADVAPI32），
无需安装 VC++ 运行库；Release 体积约 180 KB，常驻内存（工作集）约 0.4 MB。

> 内存说明：进程私有内存本就只有 ~1 MB；任务管理器早先显示的 ~9 MB 主要是
> `Shell_NotifyIcon` 间接加载的系统共享 DLL 映射页（各进程共享，并非真占用）。
> 程序在初始化后及回到空闲时会主动 `SetProcessWorkingSetSizeEx` 归还工作集，
> 任务管理器显示值降到 0.4 MB 左右；需要时（如打开菜单）系统会自动换回。
